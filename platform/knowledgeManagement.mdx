...
**Tip:** Use the columns from your CSV upload (like `intent`, `step_type`, or `brand`) as filter key-value pairs in the [Knowledge Search step](/platform/steps/knowledge-search) to retrieve specific articles. For example: `intent, FAQ` or `brand, XtendOps`.
